...
graph TD
    %% 全局样式定义（提升视觉体验）
    classDef stageStyle fill:#f0f8ff,stroke:#4169e1,stroke-width:2px,rounded:10px
    classDef stepStyle fill:#ffffff,stroke:#20b2aa,stroke-width:1px,rounded:8px
    classDef finalStyle fill:#f5f5f5,stroke:#ff6347,stroke-width:1.5px,rounded:8px

    %% 阶段一：实例化（Instantiation）
    subgraph Stage1_Instantiation [阶段一：实例化 - Instantiation]
        A["1. 加载 BeanDefinition 元数据"]:::stepStyle
        B["2. 推断构造函数（确定实例化策略）"]:::stepStyle
        C["3. 创建 Bean 实例 - createBeanInstance"]:::stepStyle
    end
    class Stage1_Instantiation stageStyle

    %% 阶段二：属性赋值（Populate）
    subgraph Stage2_Populate [阶段二：属性赋值-Populate]
        D["4. 填充属性（依赖注入-populateBean"]:::stepStyle
        E["5. 执行 Aware 接口注入<br/>(BeanName/BeanFactory/ApplicationContext 等)"]:::stepStyle
    end
    class Stage2_Populate stageStyle

    %% 阶段三：初始化（Initialization）
    subgraph Stage3_Initialization [阶段三：初始化 - Initialization]
        F["6. BeanPostProcessor前置处理<br/>▸ @PostConstruct 注解执行"]:::stepStyle
        G["7. InitializingBean 接口-afterPropertiesSet"]:::stepStyle
        H["8. 自定义 init-method 执行"]:::stepStyle
        I["9. BeanPostProcessor 后置处理<br/>▸ AOP 代理对象生成（核心）"]:::stepStyle
    end
    class Stage3_Initialization stageStyle

    %% 阶段四：销毁（Destruction）
    subgraph Stage4_Destruction [阶段四：销毁 - Destruction]
        J["10. Bean 就绪<br/>（可被业务使用）"]:::stepStyle
        K["11. 容器触发关闭 <br/> close()"]:::stepStyle
        L["12. @PreDestroy 注解方法执行"]:::stepStyle
        M["13. DisposableBean 接口 <br/> destroy"]:::stepStyle
        N["14. 自定义 destroy-method 执行"]:::stepStyle
        O["15. Bean 销毁完成<br/>（资源释放）"]:::finalStyle
    end
    class Stage4_Destruction stageStyle

    %% 流程连线（保持逻辑连贯）
    A --> B --> C --> D --> E --> F --> G --> H --> I --> J --> K --> L --> M --> N --> O

    %% 关键节点标注（突出核心知识点）
    note1["核心扩展点"]:::finalStyle -.-> I
    note2["生命周期终点"]:::finalStyle -.-> O
```
