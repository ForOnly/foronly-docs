...
1. 重载（Overload）
   重载发生在同一个类内部，指的是方法名相同，但参数列表不同。核心特征是：
   - 方法名相同
...
2. 重写（Override）
   重写发生在父类和子类之间，是子类对父类方法的重新实现。核心特征是：
   - 方法名必须相同
   - 参数列表必须相同
...
> 总结：
> 字符串常量池在 `JDK6 及之前位于方法区中`的永久代；
>
> 从 JDK7 开始，`字符串常量池被移到了堆中`；
>
> JDK8 移除了永久代，使用元空间，但`字符串常量池仍然在堆中`。
>
...
抽象类使用 abstract 修饰，通过继承的方式使用，可以有方法实现，多用于模板类和公共抽象
接口使用 interface 修饰，通过实现接口的方式使用，jdk 9 之后可以有默认的方法实现，多用于对象的能力抽象
...
JDK8 中 ConcurrentHashMap 的扩容核心是：`创建新数组 + 多线程协助迁移 + ForwardingNode 标记`
...
HashSet 的去重是借助 hashCode 和 equals 两个方法实现的，当 `hash 相同 && equals 为 true`时说明元素已经存在，不会插入
